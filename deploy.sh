...
cd ..
lftp -c "set dns:order "inet"; set ssl:verify-certificate false; set ftp:sync-mode false; open --user "${FTP_USER}" --password "${FTP_PASSWORD}" ftp://brandweeruitgeest.nl; mirror --ignore-time --continue --reverse --verbose=3 --parallel=10 public .; quit;"
