...
cd ..
lftp -c "set dns:order "inet"; set ssl:verify-certificate false; set ftp:sync-mode false; open --user "${FTP_USER}" --password "${FTP_PASSWORD}" ftp://brandweeruitgeest.nl; mirror --ignore-time --continue --reverse --overwrite --verbose=3 --parallel=8 --use-pget-n=2 public .; quit;"
